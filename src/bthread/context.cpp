...
"    cmp  $0, %rcx\n"    // 比较rcx和0，因为rcx为0，所以zf为1，rcx：第四个参数 preserve_fpu
"    je  1f\n"       // 因为zf为1，所以跳转
"    stmxcsr  (%rsp)\n"
"    fnstcw   0x4(%rsp)\n"
"1:\n"
"    movq  %rsp, (%rdi)\n"  // 将rsp保存至rdi中，rsp指向当前协程栈栈顶（原上下文），rdi为第一个入参，即ofc
"    movq  %rsi, %rsp\n"    // 将rsi保存到rsp中，rsi为第二个参数，即nfc（目标上下文），此时栈顶指针rsp指向了新的协程栈
...
".align 16\n"
"bthread_make_fcontext:\n"
"    movq  %rdi, %rax\n"  // 将第一个参数‘storage.bottom’拷贝到%rax寄存器
"    andq  $-16, %rax\n"  // 将%rax的值，即高位地址‘bottom’ & -16（补码:0xfffffff0），将地址取为16的整数倍，进行16字节对齐。
"    leaq  -0x48(%rax), %rax\n" // leaq 取64位地址指令，将%rax中保存的高位地址向下偏移72个字节后，再保存到%rax寄存器，即留出 0x48 个字节用于存放上下文 Context Data
"    movq  %rdx, 0x38(%rax)\n" // 将rdx保存至rax + 56，rdx为第三个参数，即函数fn的入口地址entry
"    stmxcsr  (%rax)\n"  // 存储 MMX 控制字和状态字，将MXCSR寄存器状态保存到rax所在位置
"    fnstcw   0x4(%rax)\n" // 存储 x87 控制字，将 FPU 控制字的当前值存储到%rax + 4
"    leaq  finish(%rip), %rcx\n" // 计算finish标志的绝对地址，将其保存至%rcx寄存器中
"    movq  %rcx, 0x40(%rax)\n" // 将rcx保存到rax + 64，finish刚好位于启动函数上方 —> 启动函数执行完以后就会执行finish处的代码，而finish会call _exit结束进程。
"    ret \n" // rax就是上述的基点（栈顶），返回的类型为fcontext_t
"finish:\n"
"    xorq  %rdi, %rdi\n" // 退出码是0
"    call  _exit@PLT\n" // call _exit结束进程
"    hlt\n"
